...
void InterpolateGrid(const Data *data, const Grid *grid, int *vars, double x1, double x2, double x3, double *v){
    ...
    DIM_EXPAND(int gr_nx1 = grid->np_tot[IDIR];,
             int gr_nx2 = grid->np_tot[JDIR];,
             int gr_nx3 = grid->np_tot[KDIR];);
    DIM_EXPAND(double * gr_x1 = grid->x[IDIR];,
             double * gr_x2 = grid->x[JDIR];,
             double * gr_x3 = grid->x[KDIR];);

    /* Find left indices */
    DIM_EXPAND(il = hunter2(gr_x1, gr_nx1, x1);,
    ...
    DIM_EXPAND(xx = (x1 - gr_x1[il]) / (gr_x1[il + 1] - gr_x1[il]);,
...
#else
                DIM_EXPAND(x1[lcount] = CART_1(cx1, cx2, cx3);,
